...
var usernames = ["sayem", "sayef", "shehab", "inverse"];

// Pathnames of the SSL key and certificate files to use for
// HTTPS connections.

const keyFilePath = "/etc/letsencrypt/live/jannatenterprise.com/privkey.pem";
const certFilePath = "/etc/letsencrypt/live/jannatenterprise.com/fullchain.pem";
...
function isUsernameInList(name) {
  var inList = false;
  var i;
...
function makeUserListMessage() {
  ...
  for (i=0; i<connectionArray.length; i++) {
    if(connectionArray[i].username === undefined) continue;
    ...
    if (message.type === 'utf8') {
      ...
      switch(msg.type) {
        ...
        case "username":
          ...
          //added to secure temporarily by sayem
          if (!isUsernameInList(msg.name)) {
            var changeMsg = {
              id: msg.id,
              type: "rejectusername",
              name: msg.name
          ...
          else if (!isUsernameUnique(msg.name)) {
            var changeMsg = {
              id: msg.id,
              type: "rejectusernamelogged",
              name: msg.name
            };
            connect.sendUTF(JSON.stringify(changeMsg));
          }
